...
注意：`vlp-video-download` 的 `subtitles` mode 只下载平台原始字幕。它不下载视频、不生成双语字幕、不代表 `vlp-orchestrator` 中“下载双语字幕”的默认路径。
...
不要把双语字幕请求直接映射为 `subtitles` mode。双语字幕是上层 pipeline 产物，应由 `vlp-orchestrator` 编排下载/转写/翻译润色。
